...
export function Countdown({ toIso }: { toIso: string }): React.ReactElement {
  ...
  return (
    <div className="flex gap-3">
      {items.map((i) => (
        <div key={i.label} className="flex-1 text-center rounded-2xl bg-ink-800/70 border border-ink-700 px-3 py-4">
          <div className="text-white font-display text-4xl tabular-nums">
            {String(i.value).padStart(2, '0')}
          </div>
          <div className="text-xs uppercase tracking-widest text-ink-300 mt-1">{i.label}</div>
        </div>
      ))}
    </div>
  );
